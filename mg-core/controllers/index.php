...
class Controllers_Index extends BaseController {
  ...
  function __construct() {
    ...
    if ($dataGroupProducts == null) {
      ...
      $saleProducts = $model->getListByUserFilter(/*MG::getSetting('countSaleProduct')*/'20', 'p.activity=1 ORDER BY '.$sort.' ASC');
      // добавление хиты продаж и новинки
      
       $newProducts = $model->getListByUserFilter('20', ' p.new=1 ORDER BY '.$sort.' ASC');
       // $recommendProducts = $model->getListByUserFilter('20', ' (p.old_price>0 || pv.old_price>0) and p.recommend=1 ORDER BY '.$sort.' ASC');
       $recommendProducts = $model->getListByUserFilter('20', ' p.recommend=1 ORDER BY '.$sort.' ASC');
