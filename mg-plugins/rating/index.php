...
class Rating {
  ...
  static function showRating($vote) {
    ...
    foreach ($entity as $rows) {
      ...
      $count = $rows['count'];
      $id = $rows['id_product'];
      $core = "<span class='info' data-id = '".$id."'><span class='mg-rating-count' data-count='".$id."'>(".$count.")</span></span> 
				";
    }
    if ($core == "") {
      $rating = $count = 0;
      $core = "<span class='info' data-id = '".$vote['id']."'><span class='mg-rating-count' data-count='".$vote['id']."'>(".$count.")</span></span>";
    }
    $core = " 
