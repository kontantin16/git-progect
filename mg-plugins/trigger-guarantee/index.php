    ...
    if ($arg['id']) {
      ...
      if ($trigger = DB::fetchArray($rez)) {
        ...
        switch ($options['place']) {
          case 'right' : $float = 'float: right';
            break;
          case 'left' : $float = 'float:left';
            break;
          case 'top' : $float = 'float:none; display: block';
        }
        $style = ' style="background-color:#'.$options['background_icon'].'; color:#'.$options['color_icon'].'; border-radius: '.$radius.';"';
        $layout = $options['layout'] == 'column' ? 'layout_column.php' : 'layout.php';
        if ($options['layout']=='vertleft'||$options['layout']=='vertright') {
          $styleTrigg = 'style="width: '.$options['width'].'%;"';
          $widthTrig = '';
        } else {
          $styleTrigg = '';
          $widthTrig = 'width: '.$options['width'].'%;';
        }
        ob_start();
        include ($layout);
